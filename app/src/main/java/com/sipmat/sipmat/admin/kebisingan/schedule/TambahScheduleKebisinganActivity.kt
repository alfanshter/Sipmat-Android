...
class TambahScheduleKebisinganActivity : AppCompatActivity(), AnkoLogger {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        binding.btntambah.setOnClickListener {
            ...
            if (tw.isNotEmpty() && tanggal_cek!=null && nama_kebisingan!=null && tahun.isNotEmpty()){
                var body = PostScheduleKebisingan(tw,tahun,nama_kebisingan,tanggal_cek)
                loading(true)
    ...
    private fun pilihkebisingan() {
        api.getkebisingan_pick().enqueue(object : Callback<KebisinganPickResponse> {
            override fun onFailure(call: Call<KebisinganPickResponse>, t: Throwable) {

            }

            override fun onResponse(
                call: Call<KebisinganPickResponse>,
                response: Response<KebisinganPickResponse>
            ) {
                if (response.isSuccessful) {
                    val kebisingan = KebisinganPickResponse.Kebisingan()
                    kebisingan.lokasi = ""
                    kebisingan.kode = "Pilih kebisingan"
                    kebisingan.id = -1
                    var spResponse: MutableList<KebisinganPickResponse.Kebisingan> =
                        response.body()!!.data as MutableList<KebisinganPickResponse.Kebisingan>
                    spResponse.add(0, kebisingan)
                    val adapter: ArrayAdapter<KebisinganPickResponse.Kebisingan> =
                        ArrayAdapter<KebisinganPickResponse.Kebisingan>(
                            this@TambahScheduleKebisinganActivity,
                            android.R.layout.simple_spinner_item,
                            spResponse
                        )
                    binding.spnKebisingan.adapter = adapter
                    binding.spnKebisingan.onItemSelectedListener =
                        object : AdapterView.OnItemSelectedListener {
                            ...
                            override fun onItemSelected(
                                ...
                                if (position == 0) {
                                    nama_kebisingan = null
                                }

                                if (position > 0) {
                                    nama_kebisingan = spResponse[position].kode
                                    id_kebisingan = spResponse[position].id
                                    lokasi_kebisingan = "${spResponse[position].kode} - ${spResponse[position].lokasi}"
                                }
                            }
