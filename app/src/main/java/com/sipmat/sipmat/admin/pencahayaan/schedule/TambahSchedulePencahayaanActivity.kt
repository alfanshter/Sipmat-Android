...
class TambahSchedulePencahayaanActivity : AppCompatActivity(), AnkoLogger {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        binding.btntambah.setOnClickListener {
            ...
            if (tw.isNotEmpty() && tanggal_cek!=null && nama_pencahayaan!=null && tahun.isNotEmpty()){
                var body = PostSchedulePencahayaanResponse(tw,tahun,nama_pencahayaan,tanggal_cek)
                loading(true)
    ...
    private fun pilihpencahayaan() {
        api.getpencahayaan_pick().enqueue(object : Callback<PencahayaanPickResponse> {
            override fun onFailure(call: Call<PencahayaanPickResponse>, t: Throwable) {

            }

            override fun onResponse(
                call: Call<PencahayaanPickResponse>,
                response: Response<PencahayaanPickResponse>
            ) {
                if (response.isSuccessful) {
                    val pencahayaan = PencahayaanPickResponse.Pencahayaan()
                    pencahayaan.lokasi = ""
                    pencahayaan.kode = "Pilih pencahayaan"
                    pencahayaan.id = -1
                    var spResponse: MutableList<PencahayaanPickResponse.Pencahayaan> =
                        response.body()!!.data as MutableList<PencahayaanPickResponse.Pencahayaan>
                    spResponse.add(0, pencahayaan)
                    val adapter: ArrayAdapter<PencahayaanPickResponse.Pencahayaan> =
                        ArrayAdapter<PencahayaanPickResponse.Pencahayaan>(
                            this@TambahSchedulePencahayaanActivity,
                            android.R.layout.simple_spinner_item,
                            spResponse
                        )
                    binding.spnPencahayaan.adapter = adapter
                    binding.spnPencahayaan.onItemSelectedListener =
                        object : AdapterView.OnItemSelectedListener {
                            ...
                            override fun onItemSelected(
                                ...
                                if (position == 0) {
                                    nama_pencahayaan = null
                                }

                                if (position > 0) {
                                    nama_pencahayaan = spResponse[position].kode
                                    id_pencahayaan = spResponse[position].id
                                    lokasi_pencahayaan = "${spResponse[position].kode} - ${spResponse[position].lokasi}"
                                }
                            }
